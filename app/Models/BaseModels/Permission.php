...
class Permission extends Model
{
    ...
    public static function getPermissionForNonAuth(){
        return Permission::where(['is_middleware'=>0])->get();
    } 

    public static function getRouteData(){
        return DB::table('tbl_permissions')
                ->select(array(DB::raw("GROUP_CONCAT(tbl_scopes.scope_name SEPARATOR ',') AS scope_name"),'tbl_permissions.slug_name','tbl_permissions.namespace', 'tbl_permissions.controller_name'
                            , 'tbl_permissions.method_name', 'tbl_permissions.method_type'
                            , 'tbl_permissions.middleware_name', 'tbl_permissions.is_middleware'))
                ->leftJoin('tbl_permssion_scopes','tbl_permissions.id','=','tbl_permssion_scopes.permission_id')
                ->leftJoin('tbl_scopes','tbl_permssion_scopes.scope_id','=','tbl_scopes.id')
                ->groupBy('tbl_permissions.id')
                ->get();
    }
